...
class CommentServiceTest {
    ...
    void setup() throws Exception {
        ...
        book1.setComments(Arrays.asList("TestComment1", "TestComment2"));
    ...
    @Test
    void insert() {
        service.insert(new ObjectId(BOOK_HEX_STRING), "BLABLABLA");
        assertEquals(3, bookRepository.findBookById(new ObjectId(BOOK_HEX_STRING)).getComments().size());
    }
    ...
    @Test
    void getByBook() {
        List<String> all = service.getByBook(new ObjectId(BOOK_HEX_STRING));
        assertEquals(2, all.size());
    }
}